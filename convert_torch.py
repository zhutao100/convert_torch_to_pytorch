...
def lua_recursive_model(module, seq):
    for m in module.modules:
        ...
        if name == 'SpatialConvolution' or name == 'nn.SpatialConvolutionMM':
            ...
            n = nn.ConvTranspose3d(m.nInputPlane, m.nOutputPlane, (m.kT, m.kW, m.kH), (m.dT, m.dW,
                                   m.dH), (m.padT, m.padW, m.padH), (m.adjT, m.adjW, m.adjH), m.groups)
            copy_param(m, n)
            add_submodule(seq, n)
...
def lua_recursive_source(module):
    ...
    for m in module.modules:
        ...
        if name == 'SpatialConvolution' or name == 'nn.SpatialConvolutionMM':
            ...
            s += ['nn.BatchNorm2d({},{},{},{}),#BatchNorm2d'.format(m.running_mean.size(0),
                                                                    m.eps, m.momentum, m.affine)]
        elif name == 'VolumetricBatchNormalization':
            s += ['nn.BatchNorm3d({},{},{},{}),#BatchNorm3d'.format(m.running_mean.size(0),
                                                                    m.eps, m.momentum, m.affine)]
        elif name == 'ReLU':
            s += ['nn.ReLU()']
        elif name == 'Sigmoid':
            ...
            s += ['nn.MaxPool2d({},{},{},ceil_mode={}),#MaxPool2d'.format((m.kW, m.kH),
                                                                          (m.dW, m.dH), (m.padW, m.padH), m.ceil_mode)]
        elif name == 'SpatialAveragePooling':
            s += ['nn.AvgPool2d({},{},{},ceil_mode={}),#AvgPool2d'.format((m.kW, m.kH),
                                                                          (m.dW, m.dH), (m.padW, m.padH), m.ceil_mode)]
        elif name == 'SpatialUpSamplingNearest':
...
def torch_to_pytorch(t7_filename, output_path=None):
    sys = platform.system()
    if sys == 'Windows':
...
class LambdaReduce(LambdaBase):
    def forward(self, input):
        return reduce(self.lambda_func,self.forward_prepare(input))
'''
    t7_pathfile = pathlib.Path(t7_filename)
    varname = t7_pathfile.stem.replace('.', '_').replace('-', '_')
    s = '{}\n\n{} = {}'.format(header, varname, s[:-2])

    if not output_path:
        output_path = pathlib.Path(t7_pathfile.parent, varname)
    else:
        output_path = pathlib.Path(output_path)
        if output_path.is_dir:
            output_path = pathlib.Path(output_path, varname)
    with open(str(output_path) + '.py', "w") as pyfile:
        pyfile.write(s)

    n = nn.Sequential()
    lua_recursive_model(model, n)
    torch.save(n.state_dict(), str(output_path) + '.pth')
